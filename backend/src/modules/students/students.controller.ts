...
export const getStudentById = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const student = await prisma.student.findUnique({
            where: { id },
            include: { school: true, guardian: true, CognitiveProfiles: true, Reports: { orderBy: { generated_at: 'desc' }, take: 1 } }
        });
...
export const createStudent = async (req: AuthRequest, res: Response) => {
    try {
        const { name, birth_date, school_id, primary_guardian_id, grade_level, tutor_ids } = req.body;
...
export const updateStudent = async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params as any;
        const { name, birth_date, primary_guardian_id, grade_level, status, tutor_ids } = req.body;
